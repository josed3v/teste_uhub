...
projectModal.addEventListener('show.bs.modal', event => {
    ...
    if(imagens.length>0){
        ...
        imagens.forEach(([src, focus], index)=>{
            ...
            html += `<div class="carousel-item ${index===0?'active':''}">
                        <img src="${src}" class="d-block w-100 modal-image" style="max-height:500px; object-fit:cover; object-position:${safeFocus};">
                     </div>`;
        });
        html += `</div>
    ...
    html += `
        <div class="d-flex gap-2 mb-3">
            <button id="modalLikeBtn" class="btn btn-outline-primary">❤️ <span id="modalLikeCount">0</span></button>
            <button id="modalColabBtn" class="btn btn-outline-success">🤝 Solicitar Colaboração</button>
        </div>
        <p>${descricao.replace(/\n/g,'<br>')}</p>
    `;
    ...
    .then(res=>res.json())
    .then(data=>{ if(data.status==='ok') atualizarLikeUI(currentProjectId, data); });

    // Botão Curtida modal
    $('#modalLikeBtn').off('click').click(()=> {
        fetch('like.php', {method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'}, body:'id='+currentProjectId})
        .then(res=>res.json())
        .then(data=>{ if(data.status==='ok') atualizarLikeUI(currentProjectId, data); });
    });

    // Botão Colaboração modal
    $('#modalColabBtn').off('click').click(()=>{
...
$('.like-btn').click(function(){
    ...
    .then(res=>res.json())
    .then(data=>{ if(data.status==='ok') atualizarLikeUI(id,data); });
});
